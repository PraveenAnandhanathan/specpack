...
Kimi uses the ``.kimi/skills/specpack-<name>/SKILL.md`` layout with
``/skill:specpack-<name>`` invocation syntax.

Includes legacy migration logic for projects initialised before Kimi
moved from dotted skill directories (``specpack.xxx``) to hyphenated
(``specpack-xxx``).
"""
...
class KimiIntegration(SkillsIntegration):
    ...
    def options(cls) -> list[IntegrationOption]:
        return [
            ...
            IntegrationOption(
                ...
                is_flag=True,
                default=False,
                help="Migrate legacy dotted skill dirs (specpack.xxx → specpack-xxx)",
            ),
        ]
    ...
    def setup(
        ...
        **opts: Any,
        ...
        # Run base setup first so hyphenated targets (specpack-*) exist,
...
def _migrate_legacy_kimi_dotted_skills(skills_dir: Path) -> tuple[int, int]:
    """Migrate legacy Kimi dotted skill dirs (specpack.xxx) to hyphenated format.
    ...
    for legacy_dir in sorted(skills_dir.glob("specpack.*")):
        if not legacy_dir.is_dir():
            continue
        if not (legacy_dir / "SKILL.md").exists():
            continue

        suffix = legacy_dir.name[len("specpack."):]
        if not suffix:
            continue

        target_dir = skills_dir / f"specpack-{suffix.replace('.', '-')}"
