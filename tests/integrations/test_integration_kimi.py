...
class TestKimiLegacyMigration:
    ...
    def test_migrate_dotted_to_hyphenated(self, tmp_path):
        ...
        legacy = skills_dir / "specpack.plan"
        ...
        assert (skills_dir / "specpack-plan" / "SKILL.md").exists()
    ...
    def test_skip_when_target_exists_different_content(self, tmp_path):
        ...
        target = skills_dir / "specpack-plan"
    ...
    def test_setup_with_migrate_legacy_option(self, tmp_path):
        ...
        skills_dir = tmp_path / ".kimi" / "skills"
        legacy = skills_dir / "specpack.oldcmd"
        legacy.mkdir(parents=True)
        ...
        assert not legacy.exists()
        assert (skills_dir / "specpack-oldcmd" / "SKILL.md").exists()
        # New skills from templates should also exist
        assert (skills_dir / "specpack-specify" / "SKILL.md").exists()
...
class TestKimiNextSteps:
    ...
    def test_next_steps_show_skill_invocation(self, tmp_path):
        """Kimi next-steps guidance should display /skill:specpack-* usage."""
        import os
        ...
        assert "/skill:specpack-constitution" in result.output
        assert "/specpack.constitution" not in result.output
